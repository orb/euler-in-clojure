...
;; reverse a number using math - uses arbitrary precision math
...
;; produce a lazy sequence of reverse sums as defined in the problem
;; statement
(defn revseq [num]
  (cons num (lazy-seq (revseq (+' num (reverse-num num))))))

;; is a number a palindrome? uses math
(defn palindrome-num? [num]
  (= num (reverse-num num)))

;; is a number a lychrel number, as defined using the first 50 rules in
;; in the problem. 
(defn lychrel? [num]
  (not (some palindrome-num?
             ;; drop the first number, since that can be a palindrome
             (take 50 (drop 1 (revseq num))))))
